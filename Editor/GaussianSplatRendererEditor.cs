...
//////
            if (GUILayout.Button("NavMesh用の当たり判定（小さくて濃いガウス）を追加"))
            {
                var renderer = (GaussianSplatRenderer)target;
                ...
                if (!renderer.EditExportData(gpuData, true)) // World座標
                {
                    Debug.LogError("Failed to get GPU data.");
                    return;
                }
                ...
                GameObject parent = new GameObject("NavMeshCollisionFromGaussian");
                ...
                for (int i = 0; i < data.Length; ++i)
                {
                    ...
                    Vector3 scale = splat.scale * 2f;
                    Quaternion rot = new Quaternion(splat.rot.x, splat.rot.y, splat.rot.z, splat.rot.w);

                    float volume = scale.x * scale.y * scale.z;
                    float opacity = splat.opacity;

                    if (volume < 0.01f && opacity > 0.3f)
                    {
                        GameObject obj = new GameObject($"NavMeshCol_{i}");
                ...
                Debug.Log($"✓ NavMesh用当たり判定オブジェクトを {count} 個追加しました！");
            }
            ...
            if (GUILayout.Button("高精度楕円体としてガウスを表示"))
            {
                var renderer = (GaussianSplatRenderer)target;
                ...
                if (!renderer.EditExportData(gpuData, true))//normal false
                {
                    Debug.LogError("Failed to get GPU data.");
                    return;
                }
                ...
                GameObject parent = new GameObject($"GaussianEllipsoids_{renderer.name}");

                for (int i = 0; i < Mathf.Min(data.Length, 1000); ++i) // 多すぎると重い
                {
                    int wordIdx = i >> 5;
                    int bitIdx = i & 31;
                    bool isDeleted = (deleted[wordIdx] & (1u << bitIdx)) != 0;
                    ...
                    ellipsoid.name = $"Ellipsoid_{i}";
                    ellipsoid.transform.position = splat.pos;

                    // 回転を正確に反映
                    ellipsoid.transform.rotation = new Quaternion(splat.rot.x, splat.rot.y, splat.rot.z, splat.rot.w);

                    // スケールを軸別に反映（楕円体）
                    ellipsoid.transform.localScale = splat.scale * 2f;

                    ellipsoid.transform.SetParent(parent.transform);

                    // 半透明の見やすいマテリアル
                    var mat = new Material(Shader.Find("Standard"));
                    mat.color = new Color(0.3f, 0.6f, 1f, splat.opacity); // 半透明ブルー
                    ...
                    ellipsoid.GetComponent<Renderer>().material = mat;
                }

                Debug.Log($"✓ 高精度楕円体として {Mathf.Min(data.Length, 1000)} ガウスを表示しました！");
            }

            if (GUILayout.Button("ガウスオブジェクト削除"))
            {
                var objs = GameObject.FindObjectsOfType<Transform>();
                foreach (var obj in objs)
                {
                    if (obj.name.StartsWith("Splat_") || obj.name.StartsWith("GaussianSplat_Instances_"))
                    {
            ...
            if (GUILayout.Button("data collect"))//world座標で位置と大きさを取得
            {
                ...
                {
                    Debug.Log($"[{i}] Pos: {data[i].pos}, Scale: {data[i].scale}");
                }
